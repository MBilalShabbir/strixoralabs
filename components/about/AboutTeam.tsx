...
const team = [
  {
    ...
    bg: "bg-violet-600/15 text-violet-300",
  },
  {
    name: "Marcus Chen",
    role: "Co-founder & CTO",
  ...
  },
  {
    ...
    bg: "bg-rose-600/15 text-rose-300",
  },
  {
    name: "Tobias Keller",
    ...
    bg: "bg-emerald-600/15 text-emerald-300",
  },
];
    ...
    <section className="py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-[clamp(1.8rem,3.5vw,2.8rem)] font-extrabold tracking-tighter text-white text-balance mb-3">
            The people who actually do the work.
          </h2>
          <p className="text-[14px] text-white/35 max-w-sm mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {team.map((member, i) => (
            <motion.div
              ...
              className="group p-5 rounded-2xl card-surface hover:border-white/10 transition-all duration-200"
            >
              <div className={cn(
              ...
              <h3 className="text-[13.5px] font-semibold text-white/90 mb-0.5">{member.name}</h3>
              <p className="text-[11.5px] text-brand-400/80 font-medium mb-3">{member.role}</p>
              <p className="text-[12.5px] text-white/35 leading-relaxed mb-4">{member.bio}</p>
              <div className="flex items-center gap-1.5">
                {[Linkedin, Twitter].map((Icon, j) => (
                  <button
                    key={j}
                    className="w-7 h-7 rounded-lg bg-white/4 border border-white/7 flex items-center justify-center text-white/25 hover:text-white/60 hover:bg-white/8 transition-all duration-200"
